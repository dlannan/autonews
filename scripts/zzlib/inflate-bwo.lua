...
function inflate.band(x,y) return bit.band(x, y) end
function inflate.rshift(x,y) return bit.rshift(x, y) end
...
function inflate.crc32(s,crc)
  if not crc32_table then
    ...
    for i=0,255 do
      local r=i
      for j=1,8 do
        r = bit.bxor(bit.rshift(r, 1), bit.band(0xedb88320, bit.bxor(bit.band(r, 1) - 1)))
      end
      crc32_table[i] = r
    end
  end
